...
const BLOG_POSTS = [
  { 
    slug: 'event-videography-guide', 
    title: 'Видеосъемка мероприятий: почему одного оператора с камерой недостаточно', 
    date: '21 Июня 2026', 
    excerpt: 'Мероприятие проходит один раз. Второго дубля не будет. Разбираем, почему профессиональная съемка события — это прежде всего подготовка, оборудование и опыт работы в непредсказуемых условиях.' 
  },
